...
def test_send_recv(rank, world_size, fileStore_path):
    ...
    if rank == 0:
        ...
        for tag in range(30):
            ...
            peer = 1
            print(f"rank {rank} sends {sendbuf}. tag {tag}")
            fut = pygloo.isend(context, sendptr, data_size, datatype, peer, tag)
            futures.append(fut)

        for fut in futures:
            fut.Wait(datetime.timedelta(seconds=30))
            print("send finished.")
        ...
        for tag in range(30):
            ...
            fut = pygloo.irecv(context, recvptr, data_size, datatype, peer, tag)
            futures.append(fut)
            print(f"rank {rank} receives {recvbuf}. tag: {tag}")
